...
def print_info():
    ...
    def highlight_x1b():
        txt3.tag_configure("underline", underline=True)
        start = "1.0"
        while True:
            start = txt3.search("\x1f", start, stopindex=END, exact=True, nocase=False, regexp=False)
            if not start:
                break
            end = f"{start}+1c"
            txt3.tag_add("underline", start, end)
            start = end
